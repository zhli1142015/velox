...
class DecimalSumAggregateAdapter : public exec::SimpleAggregateAdapter<FUNC> {
 public:
  explicit DecimalSumAggregateAdapter(TypePtr inputType, TypePtr resultType)
      : exec::SimpleAggregateAdapter<FUNC>(std::move(resultType)) {
 ...
 private:
  void addRawInputImpl(char** groups, const SelectivityVector& rows) {
    exec::VectorReader<typename FUNC::InputType::template type_at<0>> reader(
        &decodedRaw_);
    bool skipOverFlowCheck = skipOverflowCheckCount_ > accumlateRowCount_;
    if (skipOverFlowCheck) {
      rows.applyToSelected([&](auto row) {
        auto group =
            exec::Aggregate::value<typename FUNC::AccumulatorType>(groups[row]);
        bool nonNull = group->addInputFast(
            exec::Aggregate::allocator_,
            exec::optional_arg_type<
                typename FUNC::InputType::template type_at<0>>{
                &reader, (int64_t)row});
        if (nonNull) {
          exec::Aggregate::clearNull(groups[row]);
        }
      });
    } else {
      rows.applyToSelected([&](auto row) {
