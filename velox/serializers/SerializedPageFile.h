...
class SerializedPageFileWriter {
 ...
 protected:
  ...
  /// Tracks the number of rows written to the current file.
  uint64_t currentFileNumRows_{0};
  /// Tracks the number of rows pending in batch_ (not yet flushed to file).
  uint64_t pendingBatchRows_{0};
};
