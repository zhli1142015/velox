...
TEST(RowRangesTest, rowRange) {
  ...
  auto ru1 = PageRowRange::tryUnion(PageRowRange(1, 5), PageRowRange(6, 8));
  EXPECT_TRUE(ru1.has_value());
  EXPECT_EQ(ru1->toString(), "[1, 8]");

  auto ru2 = PageRowRange::tryUnion(PageRowRange(1, 5), PageRowRange(7, 8));
  EXPECT_FALSE(ru2.has_value());

  auto ri = PageRowRange::intersection(PageRowRange(2, 6), PageRowRange(4, 10));
  EXPECT_TRUE(ri.has_value());
  EXPECT_EQ(ri->toString(), "[4, 6]");

  auto ri2 = PageRowRange::intersection(PageRowRange(1, 3), PageRowRange(4, 5));
...
TEST(RowRangesTest, unionWith) {
  RowRanges a(PageRowRange(113, 241));
  RowRanges b(PageRowRange(221, 340));
  EXPECT_EQ(RowRanges::unionWith(a, b).toString(), "[[113, 340]]");

  RowRanges c(PageRowRange(113, 230));
  RowRanges d(PageRowRange(231, 340));
  EXPECT_EQ(RowRanges::unionWith(c, d).toString(), "[[113, 340]]");

  RowRanges e(PageRowRange(113, 230));
  RowRanges f(PageRowRange(232, 340));
  EXPECT_EQ(RowRanges::unionWith(e, f).toString(), "[[113, 230], [232, 340]]");

  RowRanges x;
  x.add(PageRowRange(5, 8));
  x.add(PageRowRange(10, 20));
  x.add(PageRowRange(25, 30));
  EXPECT_EQ(x.toString(), "[[5, 8], [10, 20], [25, 30]]");

  RowRanges y;
  y.add(PageRowRange(9, 18));
...
TEST(RowRangesTest, intersection) {
  ...
  RowRanges x;
  x.add(PageRowRange(0, 100));
  x.add(PageRowRange(200, 300));
  RowRanges y;
  y.add(PageRowRange(50, 250));
  EXPECT_EQ(
...
TEST(RowRangesTest, overlapAndCount) {
  RowRanges r;
  r.add(PageRowRange(0, 4));
  r.add(PageRowRange(10, 12));
...
TEST(RowRangesTest, addAdjacentChains) {
  RowRanges r;
  r.add(PageRowRange(0, 0));
  EXPECT_EQ(r.toString(), "[[0, 0]]");
  r.add(PageRowRange(1, 1));
  EXPECT_EQ(r.toString(), "[[0, 1]]");
  r.add(PageRowRange(2, 2));
...
TEST(RowRangesTest, addNonAdjacent) {
  RowRanges r;
  r.add(PageRowRange(5, 10));
  r.add(PageRowRange(12, 15));
  EXPECT_EQ(r.toString(), "[[5, 10], [12, 15]]");
  EXPECT_EQ(r.rowCount(), (10 - 5 + 1) + (15 - 12 + 1));
}

TEST(RowRangesTest, unionChainedAdjacent) {
  RowRanges a(PageRowRange(0, 3));
  RowRanges b(PageRowRange(4, 7));
  RowRanges c(PageRowRange(8, 10));
...
TEST(RowRangesTest, unionInterleaved) {
  RowRanges a;
  a.add(PageRowRange(0, 2));
  a.add(PageRowRange(10, 12));
  RowRanges b;
  b.add(PageRowRange(1, 11));
  auto u = RowRanges::unionWith(a, b);
  EXPECT_EQ(u.toString(), "[[0, 12]]");
}

TEST(RowRangesTest, intersectionTouchingEdge) {
  RowRanges a(PageRowRange(100, 200));
  RowRanges b(PageRowRange(200, 300));
...
TEST(RowRangesTest, intersectionSinglePoint) {
  RowRanges a(PageRowRange(50, 50));
  RowRanges b(PageRowRange(0, 100));
  auto inter = RowRanges::intersection(a, b);
...
TEST(RowRangesTest, overlapAcrossSegments) {
  RowRanges r;
  r.add(PageRowRange(0, 10));
  r.add(PageRowRange(20, 30));
...
TEST(RowRangeTest, differenceNoOverlap) {
  auto v = PageRowRange::difference({0, 5}, {10, 15});
  EXPECT_EQ(v.size(), 1);
...
TEST(RowRangeTest, differenceFullOverlap) {
  auto v = PageRowRange::difference({0, 5}, {0, 5});
  EXPECT_TRUE(v.empty());
}

TEST(RowRangeTest, differenceOverlapStart) {
  auto v = PageRowRange::difference({0, 5}, {0, 2});
  EXPECT_EQ(v.size(), 1);
...
TEST(RowRangeTest, differenceOverlapEnd) {
  auto v = PageRowRange::difference({0, 5}, {3, 5});
  EXPECT_EQ(v.size(), 1);
  EXPECT_EQ(v[0].from_, 0);
  EXPECT_EQ(v[0].to_, 2);
}
...
TEST(RowRangeTest, differenceOverlapMiddle) {
  auto v = PageRowRange::difference({0, 10}, {3, 7});
  EXPECT_EQ(v.size(), 2);
  EXPECT_EQ(v[0].from_, 0);
  EXPECT_EQ(v[0].to_, 2);
...
TEST(RowRangesTest, multipleValidRanges) {
  RowRanges rs;
  rs.add(PageRowRange(8, 9));
  rs.add(PageRowRange(12, 14));
  rs.add(PageRowRange(20, 25));
...
TEST(RowRangesTest, startInsideGap) {
  RowRanges rs;
  rs.add(PageRowRange(5, 9));
  rs.add(PageRowRange(15, 20));
...
TEST(RowRangesTest, unionDisjointNonAdjacent) {
  RowRanges a(PageRowRange(0, 2));
  RowRanges b(PageRowRange(5, 7));
  auto u = RowRanges::unionWith(a, b);
  EXPECT_EQ(u.toString(), "[[0, 2], [5, 7]]");
}

TEST(RowRangesTest, unionAdjacentRanges) {
  RowRanges a(PageRowRange(0, 4));
  RowRanges b(PageRowRange(5, 9));
  auto u = RowRanges::unionWith(a, b);
  EXPECT_EQ(u.toString(), "[[0, 9]]");
}

TEST(RowRangesTest, unionOverlappingRanges) {
  RowRanges a(PageRowRange(0, 5));
  RowRanges b(PageRowRange(3, 10));
  auto u = RowRanges::unionWith(a, b);
  EXPECT_EQ(u.toString(), "[[0, 10]]");
}
...
TEST(RowRangesTest, unionMultipleRanges) {
  RowRanges a;
  a.add(PageRowRange(1, 2));
  a.add(PageRowRange(5, 6));
  RowRanges b;
  b.add(PageRowRange(2, 5));
  b.add(PageRowRange(8, 9));
  auto u = RowRanges::unionWith(a, b);
...
TEST(RowRangesTest, complementMultipleRanges) {
  RowRanges src;
  src.add(PageRowRange(1, 2));
  src.add(PageRowRange(5, 6));
...
TEST(RowRangesTest, intersectWithCompleteOverlap) {
  RowRanges a(PageRowRange(2, 8));
  RowRanges b(PageRowRange(2, 8));
  a.intersectWith(b);
...
TEST(RowRangesTest, intersectWithMultipleIntervals) {
  RowRanges a;
  a.add(PageRowRange(0, 3));
  a.add(PageRowRange(5, 9));
  RowRanges b;
  b.add(PageRowRange(2, 6));
  b.add(PageRowRange(8, 10));
  a.intersectWith(b);
