 ...
 public:
  ...
  ColumnPageIndex(
    ...
    for (size_t i = 0; i < n; ++i) {
      int64_t start =
          offsetIndex_.page_locations()->at(i).first_row_index().value();
      int64_t end =
          (i + 1 < n ? offsetIndex_.page_locations()
                           ->at(i + 1)
                           .first_row_index()
                           .value()
                     : static_cast<int64_t>(totalRows_));
  ...
  std::unique_ptr<dwio::common::ColumnStatistics> buildColumnStatisticsForPage(
    ...
    const auto nullCounts = columnIndex_.null_counts_ref();
    std::optional<uint64_t> valueCount = nullCounts.has_value()
        ? std::optional<uint64_t>(
              pageRowCount_.at(index) - nullCounts.value().at(index))
        : std::nullopt;
    std::optional<bool> hasNull = nullCounts.has_value()
        ? std::optional<bool>(nullCounts.value().at(index) > 0)
        : std::nullopt;
