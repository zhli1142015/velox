...
#include "velox/connectors/hive/FileConfig.h" // @manual=//velox/connectors/hive:velox_hive_connector_parquet
...
class ParquetTableScanTest : public HiveConnectorTestBase {
 protected:
  ...
  std::shared_ptr<Task> assertSelect(
      ...
      const std::string& sql) {
    return assertSelect(
        {makeSplit(splitFilePath_, splitPartitionKeys_, splitInfoColumns_)},
  ...
  std::shared_ptr<Task> assertSelectWithFilter(
    ...
    return AssertQueryBuilder(plan, duckDbQueryRunner_)
        ...
        .connectorSessionProperty(
            kHiveConnectorId,
            FileConfig::kParquetFilterColumnIndexEnabledSession,
            "true")
        .splits(splits)
        .assertResults(sql);
  }
  ...
  std::shared_ptr<Task> assertSelectWithFilterAndAgg(
      ...
      const std::vector<std::string>& groupingKeys,
      const std::string& sql) {
    return assertSelectWithFilterAndAgg(
        {makeSplit(splitFilePath_, splitPartitionKeys_, splitInfoColumns_)},
        std::move(outputColumnNames),
        filters,
        aggregates,
  ...
  void loadData(
    ...
    splitFilePath_ = filePath;
    splitPartitionKeys_ = partitionKeys;
    splitInfoColumns_ = infoColumns;
    rowType_ = rowType;
    createDuckDbTable({data});
  }
 ...
 private:
  ...
  RowTypePtr rowType_;
  // Split parameters used by the page index tests. Loaded by loadData().
  std::string splitFilePath_;
  std::optional<std::unordered_map<std::string, std::optional<std::string>>>
      splitPartitionKeys_;
  std::optional<std::unordered_map<std::string, std::string>> splitInfoColumns_;
